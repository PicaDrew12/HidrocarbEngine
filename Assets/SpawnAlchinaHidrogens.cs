...
public class SpawnAlchinaCarbons : MonoBehaviour
{
    ...
    public spawnAlchina spawnAlchenaScript;
    ...
    void Start()
    {
        ...
        spawnAlchenaScript = GameObject.FindGameObjectWithTag("spawnAlchina").GetComponent<spawnAlchina>();
    ...
    void SpawnOnAllSides(int colisionCount)
    {
        ...
        {
            if (iterationNumber == 1) // Linear geometry for triple bond (alkyne)
            {
                if(carbonIndex == 0)
                {
                    flip = -1;
                ...
                Debug.Log("omg");
                Quaternion rotation = Quaternion.Euler(0, 90*flip, 0);
                Vector3 position = sphereTransform.position + rotation * Vector3.forward;

                spawnedHydrogen = Instantiate(hydrogen, position + new Vector3(0, 0.635f *(-flip), 0), rotation);
                hydrogenConection = Instantiate(conection, position + rotation * new Vector3(0, 0.257f * (-flip), -0.5f), Quaternion.Euler(0, 0, 50));

                spawnedHydrogen.transform.parent = sphereTransform;
                hydrogenConection.transform.parent = sphereTransform;
                
            }
            else if (iterationNumber == 2) // Two connections
            {
                flip = (i % 2 == 0) ? 1 : -1;
            ...
            else // Three connections
            {
                if (i == 1)
                {
                    rotationZ = 90;
                    rotationX = 0;
                    flip = (gameObject == spawnAlchenaScript.carbonList[0]) ? 1 : -1;
